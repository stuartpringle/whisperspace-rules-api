export type StatusDeltaMap = Record<string, number>;

export function parseStatusEffects(raw: string | undefined | null): StatusDeltaMap {
  const deltas: StatusDeltaMap = {};
...
export function mergeStatusDeltas(statuses: Array<string | undefined | null>): StatusDeltaMap {
  const out: StatusDeltaMap = {};
...
export function computeStatusEffects(statuses: Array<string | undefined | null>) {
  const deltas = mergeStatusDeltas(statuses);
  return { deltas };
}

export function applyStatusToDerived<T extends Record<string, any>>(
  derived: T,
  deltas: StatusDeltaMap
): T {
  ...
  for (const [k, v] of Object.entries(deltas)) {
    ...
    if (k === "cool_under_fire" || k === "speed" || k === "carrying_capacity") continue;
